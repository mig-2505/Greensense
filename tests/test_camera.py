...
PASTA_FOTOS = "dados/fotos_medicoes_tests"
...
def main():
    ...
    while True:
        ...
        if tecla == ord('q'):
            ...
            contador_fotos += 1
            nome_arquivo = f"foto_{contador_fotos}_{datetime.now().strftime('%d-%m-%Y_%H-%M-%S')}.jpg"

            # os.path.join junta pasta + nome do arquivo do jeito certo
            caminho_completo = os.path.join(PASTA_FOTOS, nome_arquivo)

            # Teste 1: salvar como PNG
            caminho_png = os.path.splitext(caminho_completo)[0] + ".png"

            sucesso_png = cv2.imwrite(
                caminho_png,
                frame
            )
            ...
            print()
            print("===== TESTE PNG =====")
            print("Caminho:", caminho_png)
            print("Sucesso:", sucesso_png)
            print("Existe:", os.path.exists(caminho_png))
            print("=====================")
